...
基于 Go 语言开发的图片托管服务，使用 Cloudflare R2 作为存储后端。

## 功能特性
- 无限容量，上传图片到 Cloudflare R2 对象存储
- 轻量级要求，内存占用小于 10MB
- 支持管理员登录，查看上传记录和删除图片
...
1. Cloudflare R2 准备工作：
   - 登录 Cloudflare Dashboard，创建 R2 存储桶
   - 创建 R2 API 令牌（需要对象读写权限）
   - 记录 Account ID、Access Key ID 和 Access Key Secret
   - 配置存储桶的公开访问 URL（可通过自定义域名或 R2 公开访问地址）
...
```json
{
    "r2": {
        ...
        "publicUrl": "https://your-r2-public-url.com"
    },
    "admin": {
        "username": "nodeseeker",
...
**R2 存储配置**
- `r2.accountId`：Cloudflare 账户 ID
- `r2.accessKeyId`：R2 API 令牌的 Access Key ID
- `r2.accessKeySecret`：R2 API 令牌的 Access Key Secret
- `r2.bucketName`：R2 存储桶名称
- `r2.publicUrl`：R2 存储桶的公开访问 URL
...
- 2025-12-08：v0.2.0 **存储后端切换为 Cloudflare R2**，完美支持动态图片（GIF/动态WebP），与旧版数据库不兼容

## 常见问题

1. 上传失败：
   - 检查 R2 配置是否正确（Account ID、Access Key 等）
   - 确认 R2 API 令牌具有对象读写权限
   - 验证 SSL 证书是否正确配置
...
5. 图片嵌入其他网站：
   ...
   - 对于 GIF 动态图片，R2 存储完美保持原格式，无需特殊处理